...
class InferenceModelOutsideDecoder(model.SockeyeModelOutsideDecoder):
    """
    Supports operations used for inference/decoding including document context:
    ...
    def __init__(self,
        ...
        self.softmax_temperature = softmax_temperature
        self.max_input_length, self.get_max_output_length = \
            models_max_input_output_length([self],
                                           max_output_length_num_stds,
                                           forced_max_output_len=forced_max_output_len)
    ...
    def _get_default_bucket_key(self) -> data_io.BucketDoc:
        max_output_len = self.get_max_output_length(self.max_input_length)
        src_pre_bkts = [self.max_input_length
                        for _ in range(self.config.doc_context_config.window_config.src_pre)]
        src_nxt_bkts = [self.max_input_length
                        for _ in range(self.config.doc_context_config.window_config.src_nxt)]

        tar_pre_bkts = [max_output_len
                        for _ in range(self.config.doc_context_config.window_config.tar_pre)]
        tar_nxt_bkts = [max_output_len
                        for _ in range(self.config.doc_context_config.window_config.tar_nxt)]
        return data_io.BucketDoc(
                src_bkt=self.max_input_length,
                tar_bkt=max_output_len,
                src_pre_bkts=src_pre_bkts,
                src_nxt_bkts=src_nxt_bkts,
                tar_pre_bkts=tar_pre_bkts,
                tar_nxt_bkts=tar_nxt_bkts
        )

    def _get_encoder_module(self) -> Tuple[mx.mod.BucketingModule, data_io.BucketDoc]:
        """
        Returns a BucketingModule for the encoder. Given a source sequence and additional
        context sentences, it returns the initial decoder states of the model.
        ...
        def sym_gen(bucket: data_io.BucketDoc):
            ...
            (source_embed,
             source_embed_length,
             source_embed_seq_len) = self.embedding_source.encode(source, source_length, bucket.src_len)
            ...
            data_names = [C.SOURCE_NAME]
            for name_format, size in zip([doc_context.SOURCE_PRE_NAME, doc_context.SOURCE_NXT_NAME,
                                          doc_context.TARGET_PRE_NAME, doc_context.TARGET_NXT_NAME],
                                         self.config.doc_context_config.window_config.sizes):
                data_names.extend([name_format % i for i in range(size)])
            label_names = []  # type: List[str]
            return mx.sym.Group(decoder_init_states), data_names, label_names

        default_bucket_key = self._get_default_bucket_key()
        module = mx.mod.BucketingModule(sym_gen=sym_gen,
                                        default_bucket_key=default_bucket_key,
                                        context=self.context)
        return module, default_bucket_key

    def _get_decoder_module(self) -> Tuple[mx.mod.BucketingModule, data_io.BucketDoc]:
        """
        Returns a BucketingModule for a single decoder step.
        Given previously predicted word and previous decoder states, it returns
        a distribution over the next predicted word and the next decoder states.
        ...
        def sym_gen(bucket: data_io.BucketDoc):
            ...
            """
            # Obtain lengths
            source_seq_len, decode_step = bucket.src_len, bucket.tar_len
            source_embed_seq_len = self.embedding_source.get_encoded_seq_len(source_seq_len)
            source_encoded_seq_len = self.encoder.get_encoded_seq_len(source_embed_seq_len)
    ...
    def _get_encoder_data_shapes(self, bucket_key: data_io.BucketDoc, batch_size: int) -> List[mx.io.DataDesc]:
        """
        Returns data shapes of the encoder module.

        :param bucket_key: Maximum input lengths considering all data types, that is, current source length as well as,
                           context sentences.
        :return: List of data descriptions.
        """
        shapes = [mx.io.DataDesc(name=C.SOURCE_NAME,
                                 shape=(batch_size, bucket_key.src_len, self.num_source_factors),
                                 layout=C.BATCH_MAJOR)]
        all_sizes = self.config.doc_context_config.window_config.sizes
        names = [doc_context.SOURCE_PRE_NAME, doc_context.SOURCE_NXT_NAME,
                 doc_context.TARGET_PRE_NAME, doc_context.TARGET_NXT_NAME]
        buckets = [bucket_key.src_pre_lens, bucket_key.src_nxt_lens,
                   bucket_key.tar_pre_lens, bucket_key.tar_nxt_lens]
        for size, name, bucket in zip(all_sizes, names, buckets):
            for i, bucket_length in zip(range(size), bucket):
                shapes.append(
                        mx.io.DataDesc(name=name % i,
                                       shape=(batch_size, bucket_length, 1),
                                       layout=C.BATCH_MAJOR)
                )
        return shapes

    @lru_cache(maxsize=None)
    def _get_decoder_data_shapes(self, bucket_key: data_io.BucketDoc, batch_beam_size: int) -> List[mx.io.DataDesc]:
        """
        Returns data shapes of the decoder module.
        ...
        :return: List of data descriptions.
        """
        source_max_length, target_max_length = bucket_key.src_len, bucket_key.tar_len
        return [mx.io.DataDesc(name=C.TARGET_NAME, shape=(batch_beam_size,),
                               layout="NT")] + self.decoder.state_shapes(batch_beam_size,
                                                                         target_max_length,
                                                                         self.encoder.get_encoded_seq_len(
    ...
    def run_encoder(self,
                    source: mx.nd.NDArray,
                    source_pre: List[mx.nd.NDArray],
                    source_nxt: List[mx.nd.NDArray],
                    target_pre: List[mx.nd.NDArray],
                    target_nxt: List[mx.nd.NDArray],
                    bucket_key: data_io.BucketDoc) -> 'ModelState':
        """
        Runs forward pass of the encoder.
        Encodes source given source length and bucket key.
        Returns encoder representation of the source, source_length, initial hidden state of decoder RNN,
        ...
        :param source: Integer-coded input tokens. Shape (batch_size, source length, num_source_factors).
        :param source_pre: Integer-coded input tokens for all previous source sentences.
                           Shape (batch_size, source_pre_length_i, 1)
        :param source_nxt: Integer-coded input tokens for all next source sentences.
                           Shape (batch_size, source_nxt_length_j, 1)
        :param target_pre: Integer-coded input tokens for all previous target sentences.
                           Shape (batch_size, target_pre_length_k, 1)
        :param target_nxt: Integer-coded input tokens for all next target sentences.
                           Shape (batch_size, target_nxt_length_m, 1)
        :param bucket_key: Bucket key.
        :return: Initial model state.
        """
        data = [source] + source_pre + source_nxt + target_pre + target_nxt
        batch_size = source.shape[0]
        batch = mx.io.DataBatch(data=data,
                                label=None,
                                bucket_key=bucket_key,
                                provide_data=self._get_encoder_data_shapes(bucket_key, batch_size))
...
class InferenceModelInsideDecoder(model.SockeyeModelInsideDecoder):
    """
    Supports operations used for inference/decoding including document context:
    ...
    def __init__(self,
                 config: model.ModelConfigInsideDecoder,
                 params_fname: str,
                 context: mx.context.Context,
                 beam_size: int,
                 softmax_temperature: Optional[float] = None,
    ...
    def _get_encoder_module(self) -> Tuple[mx.mod.BucketingModule, data_io.BucketDoc]:
        ...
        def sym_gen(bucket: data_io.BucketDoc):
            ...
            decoder_init_states = self.decoder.init_states_doc(source_encoded,
                                                               source_encoded_length,
                                                               source_encoded_seq_len,
                                                               doc_enc,
                                                               doc_enc_lengths,
    ...
    def _get_decoder_module(self) -> Tuple[mx.mod.BucketingModule, data_io.BucketDoc]:
        ...
        def sym_gen(bucket: data_io.BucketDoc):
            ...
            doc_seq_lens = []  # type: List[int]
            for emb, lengths in zip([self.embedding_doc_source] * 2 + [self.embedding_doc_target] * 2,
                                    [bucket.src_pre_lens, bucket.src_nxt_lens, bucket.tar_pre_lens, bucket.tar_nxt_lens]):
                doc_seq_lens += [emb.get_encoded_seq_len(length) for length in lengths]
            ...
            (target_decoded,
             attention_probs,
             states) = self.decoder.decode_step_doc(decode_step,
                                                    target_embed_prev,
                                                    source_encoded_seq_len,
                                                    doc_seq_lens,
                                                    *states)
    ...
    def _get_decoder_data_shapes(self, bucket_key: data_io.BucketDoc, batch_beam_size: int) -> List[mx.io.DataDesc]:
        ...
        """
        source_max_length, target_max_length = bucket_key.src_len, bucket_key.tar_len
        doc_lengths = sum(bucket_key.to_tuple()) - source_max_length - target_max_length
        number_doc_sentences = len(bucket_key.to_tuple()) - 2
        # depth is from embedding size, simply crawl from one
        additional_emb_depth = [emb.get_num_hidden() for emb in [self.embedding_doc_source, self.embedding_doc_target]
                                if emb is not None][0]
        return [mx.io.DataDesc(name=C.TARGET_NAME, shape=(batch_beam_size,),
                               layout="NT")] \
               + self.decoder.state_shapes_doc(batch_beam_size,
                                               target_max_length,
                                               self.encoder.get_encoded_seq_len(source_max_length),
                                               self.encoder.get_num_hidden(),
                                               doc_lengths,
                                               additional_emb_depth,
                                               number_doc_sentences)
...
def load_models(context: mx.context.Context,
                ...
                sampling: bool = False,
                doc_context_method: Optional[str] = None) -> Tuple[List[InferenceModel],
                                                                   List[vocab.Vocab],
                                                                   vocab.Vocab]:
    """
    ...
    :param doc_context_method: Method of context-aware architecture; used to load correct configuration class.
    :return: List of models, source vocabulary, target vocabulary, source factor vocabularies.
    """
    logger.info("Loading %d model(s) from %s ...", len(model_folders), model_folders)
    load_time_start = time.time()
    models = []  # type: List[Union[InferenceModel, InferenceModelOutsideDecoder]]
    ...
    # select type of model for inference computation
    model_selection = {
        doc_context.OUTSIDE_DECODER:
            (model.SockeyeModelOutsideDecoder.load_config, InferenceModelOutsideDecoder),
        doc_context.INSIDE_DECODER_PARALLEL:
            (model.SockeyeModelInsideDecoder.load_config, InferenceModelInsideDecoder),
        doc_context.INSIDE_DECODER_SEQUENTIAL:
            (model.SockeyeModelInsideDecoder.load_config, InferenceModelInsideDecoder),
    }
    selection = model_selection.get(doc_context_method, (model.SockeyeModel, InferenceModel))
    ...
    for model_folder, checkpoint in zip(model_folders, checkpoints):
        ...
        model_config = selection[0](os.path.join(model_folder, C.CONFIG_NAME))
        ...
        inference_model = selection[1](config=model_config,
...
def models_max_input_output_length(models: List[Union[InferenceModel, InferenceModelOutsideDecoder]],
                                   num_stds: int,
...
class TranslatorInputDoc(TranslatorInput):
    """
    Includes additional context sentences for context-aware architectures.
    ...
    __slots__ = ('sentence_id', 'tokens', 'factors', 'constraints', 'avoid_list', 'pass_through_dict',
                 'src_pre_tokens', 'src_nxt_tokens', 'tar_pre_tokens', 'tar_nxt_tokens')

    def __init__(self,
                 sentence_id: SentenceId,
                 tokens: Tokens,
                 src_pre_tokens: List[Tokens],
                 src_nxt_tokens: List[Tokens],
                 tar_pre_tokens: List[Tokens],
                 tar_nxt_tokens: List[Tokens],
                 factors: Optional[List[Tokens]] = None,
                 constraints: Optional[List[Tokens]] = None,
                 avoid_list: Optional[List[Tokens]] = None,
                 pass_through_dict: Optional[Dict] = None) -> None:
        super().__init__(sentence_id=sentence_id, tokens=tokens, factors=factors,
                         constraints=constraints, avoid_list=avoid_list, pass_through_dict=pass_through_dict)
        self.src_pre_tokens = src_pre_tokens
        self.src_nxt_tokens = src_nxt_tokens
        self.tar_pre_tokens = tar_pre_tokens
        self.tar_nxt_tokens = tar_nxt_tokens

    @property
    def max_len_incl_context(self) -> int:
        return max(max((len(src_pre) for src_pre in self.src_pre_tokens), default=0),
                   max((len(src_nxt) for src_nxt in self.src_nxt_tokens), default=0),
                   len(self),
                   max((len(tar_pre) for tar_pre in self.tar_pre_tokens), default=0),
                   max((len(tar_nxt) for tar_nxt in self.tar_nxt_tokens), default=0))

    def chunks(self, chunk_size: int) -> Generator['TranslatorInputDoc', None, None]:
        """
        Takes a TranslatorInputDoc (itself) and yields TranslatorInputsDoc for chunks of size chunk_size.

        :param chunk_size: The maximum size of a chunk.
        :return: A generator of TranslatorInputDocs, one for each chunk created.
        """
        ...
        logger.info('Perform Chunking due to max-length')
        for chunk_id, i in enumerate(range(0, self.max_len_incl_context, chunk_size)):
            factors = [factor[i:i + chunk_size] for factor in self.factors] if self.factors is not None else None
            # Constrained decoding is not supported for chunked TranslatorInputs. As a fall-back, constraints are
            # assigned to the first chunk
            constraints = self.constraints if chunk_id == 0 else None
            pass_through_dict = self.pass_through_dict if chunk_id == 0 else None
            tokens_src_pre = [src_pre[i:i + chunk_size] for src_pre in self.src_pre_tokens]
            tokens_src_nxt = [src_nxt[i:i + chunk_size] for src_nxt in self.src_nxt_tokens]
            tokens_tar_pre = [tar_pre[i:i + chunk_size] for tar_pre in self.tar_pre_tokens]
            tokens_tar_nxt = [tar_nxt[i:i + chunk_size] for tar_nxt in self.tar_nxt_tokens]
            yield TranslatorInputDoc(sentence_id=self.sentence_id,
                                     tokens=self.tokens[i:i + chunk_size],
                                     src_pre_tokens=tokens_src_pre,
                                     src_nxt_tokens=tokens_src_nxt,
                                     tar_pre_tokens=tokens_tar_pre,
                                     tar_nxt_tokens=tokens_tar_nxt,
                                     factors=factors,
                                     constraints=constraints,
                                     avoid_list=self.avoid_list,
                                     pass_through_dict=pass_through_dict)

    def with_eos(self) -> 'TranslatorInputDoc':
        """
        :return: A new translator input with EOS appended to the tokens and factors.
        """
        src_pre_eos = [s_pre + [C.EOS_SYMBOL] for s_pre in self.src_pre_tokens]
        src_nxt_eos = [s_nxt + [C.EOS_SYMBOL] for s_nxt in self.src_nxt_tokens]
        tar_pre_eos = [t_pre + [C.EOS_SYMBOL] for t_pre in self.tar_pre_tokens]
        tar_nxt_eos = [t_nxt + [C.EOS_SYMBOL] for t_nxt in self.tar_nxt_tokens]
        return TranslatorInputDoc(sentence_id=self.sentence_id,
                                  tokens=self.tokens + [C.EOS_SYMBOL],
                                  src_pre_tokens=src_pre_eos,
                                  src_nxt_tokens=src_nxt_eos,
                                  tar_pre_tokens=tar_pre_eos,
                                  tar_nxt_tokens=tar_nxt_eos,
                                  factors=[factor + [C.EOS_SYMBOL] for factor in
                                           self.factors] if self.factors is not None else None,
                                  constraints=self.constraints,
                                  avoid_list=self.avoid_list,
...
def make_input_from_plain_string_doc(sentence_id: SentenceId,
                                     string: str,
                                     src_pre_tokens: List[Tokens],
                                     src_nxt_tokens: List[Tokens],
                                     tar_pre_tokens: List[Tokens],
                                     tar_nxt_tokens: List[Tokens]) -> TranslatorInputDoc:
    """
    Returns a TranslatorInputDoc object from plain strings.

    :param sentence_id: Sentence id.
    :param string: An input string for the current source sentence.
    :param src_pre_tokens: Previous source strings.
    :param src_nxt_tokens: Next source strings.
    :param tar_pre_tokens: Previous target strings.
    :param tar_nxt_tokens: Next target strings.
    :return: A TranslatorInputDoc.
    """
    return TranslatorInputDoc(sentence_id,
                              tokens=list(data_io.get_tokens(string)),
                              src_pre_tokens=src_pre_tokens,
                              src_nxt_tokens=src_nxt_tokens,
                              tar_pre_tokens=tar_pre_tokens,
                              tar_nxt_tokens=tar_nxt_tokens,
                              factors=None)
...
def make_input_from_factored_string_doc(sentence_id: SentenceId,
                                        factored_string: str,
                                        src_pre: List[Tokens],
                                        src_nxt: List[Tokens],
                                        tar_pre: List[Tokens],
                                        tar_nxt: List[Tokens],
                                        translator: 'TranslatorOutsideDecoder',
                                        delimiter: str = C.DEFAULT_FACTOR_DELIMITER) -> TranslatorInput:
    """
    Returns a TranslatorInputDoc object from a string with factor annotations on a token level, separated by delimiter.
    Moreover, we include context sentences as additional information.
    If translator does not require any source factors, the string is parsed as a plain token string.

    :param sentence_id: Sentence id.
    :param factored_string: An input string with additional factors per token, separated by delimiter.
    :param src_pre: List of previous source sentence tokens.
    :param src_nxt: List of next source sentence tokens.
    :param tar_pre: List of previous target sentence tokens.
    :param tar_nxt: List of next target sentence tokens.
    :param translator: A translator object.
    :param delimiter: A factor delimiter. Default: '|'.
    :return: A TranslatorInput.
    """
    ...
    if model_num_source_factors == 1:
        return make_input_from_plain_string_doc(sentence_id=sentence_id,
                                                string=factored_string,
                                                src_pre_tokens=src_pre,
                                                src_nxt_tokens=src_nxt,
                                                tar_pre_tokens=tar_pre,
                                                tar_nxt_tokens=tar_nxt)
    raise NotImplementedError("Source factors are not supported in inference mode.")
...
def make_input_from_multiple_strings_doc(sentence_id: SentenceId,
                                         strings: str,
                                         src_pre: List[Tokens],
                                         src_nxt: List[Tokens],
                                         tar_pre: List[Tokens],
                                         tar_nxt: List[Tokens]) -> TranslatorInputDoc:
    """
    Returns a TranslatorInputDoc object including context sentences.

    :param sentence_id: Sentence id.
    :param strings: A list of strings representing a factored input sequence.
    :param src_pre: List of previous source sentences as strings.
    :param src_nxt: List of next source sentences as strings.
    :param tar_pre: List of previous target sentences as strings.
    :param tar_nxt: List of next target sentences as strings.
    :return: A TranslatorInput.
    """
    return TranslatorInputDoc(sentence_id=sentence_id,
                              tokens=list(data_io.get_tokens(strings)),
                              src_pre_tokens=src_pre,
                              src_nxt_tokens=src_nxt,
                              tar_pre_tokens=tar_pre,
                              tar_nxt_tokens=tar_nxt)
...
IndexedTranslatorInputDoc = NamedTuple('IndexedTranslatorInputDoc', [
    ('input_idx', int),
    ('chunk_idx', int),
    ('translator_input', TranslatorInputDoc)
])
"""
Translation of a chunk of a sentence.
...
:param input_idx: Internal index of translation requests to keep track of the correct order of translations.
:param chunk_idx: The index of the chunk. Used when TranslatorInputs get split across multiple chunks.
:param translator_input: The translator input with context sentences.
"""
...
class TranslatorOutsideDecoder:
    """
    Translator uses one or several models to translate input with additional context sentences. These additional inputs
    are surrounding sentences from the current position. For target context usage we recommend using a baseline model
    to compute a hypothesis which is then used as future (or next) target context information.
    ...
    def __init__(self,
                 ...
                 beam_prune: float,
                 beam_search_stop: str,
                 models: List[InferenceModelOutsideDecoder],
                 source_vocabs: List[vocab.Vocab],
                 target_vocab: vocab.Vocab,
                 nbest_size: int = 1,
                 restrict_lexicon: Optional[lexicon.TopKLexicon] = None,
        ...
        if bucket_source_width > 0:
            self.buckets_source = data_io.define_buckets(self._max_input_length, step=bucket_source_width)
            self.buckets_target = data_io.define_buckets(self.models[0].get_max_output_length(self._max_input_length),
                                                         step=bucket_source_width)
        else:
            self.buckets_source = [self._max_input_length]
            self.buckets_target = [self.models[0].get_max_output_length(self._max_input_length)]
        ...
        logger.info("Translator (Outside-decoder) (%d model(s) beam_size=%d beam_prune=%s beam_search_stop=%s "
                    "nbest_size=%s ensemble_mode=%s max_batch_size=%d buckets_source=%s avoiding=%d)",
                    len(self.models),
                    self.beam_size,
                    'off' if not self.beam_prune else "%.2f" % self.beam_prune,
    ...
    def translate(self, trans_inputs: List[TranslatorInputDoc], fill_up_batches: bool = True) -> List[TranslatorOutput]:
        """
        Batch-translates a list of TranslatorInputDocs, returns a list of TranslatorOutputs. We consider further context
        sentences during the translation process using context-aware architectures.
        ...
        :param trans_inputs: List of TranslatorInputDocs as returned by make_input_doc().
        :param fill_up_batches: If True, underfilled batches are padded to Translator.max_batch_size.
        :return: List of translation results.
        """
        num_inputs = len(trans_inputs)
        ...
        # split into chunks
        input_chunks = []  # type: List[IndexedTranslatorInputDoc]
        for trans_input_idx, trans_input in enumerate(trans_inputs):
            # bad input
            if isinstance(trans_input, BadTranslatorInput):
                translated_chunks.append(IndexedTranslation(input_idx=trans_input_idx, chunk_idx=0,
            ...
            # empty input
                ...
                if self.source_with_eos:
                    max_input_length_without_eos = self.max_input_length
                    # oversized input
                    if trans_input.max_len_incl_context > max_input_length_without_eos:
                        logger.debug(
                            "Input %s has length (%d) that exceeds max input length (%d). "
                            "Splitting into chunks of size %d.",
                            trans_input.sentence_id, len(trans_input.tokens),
                        ...
                        chunks = [trans_input_chunk.with_eos()
                                  for trans_input_chunk in trans_input.chunks(max_input_length_without_eos)]
                        input_chunks.extend([IndexedTranslatorInputDoc(trans_input_idx, chunk_idx, chunk_input)
                                             for chunk_idx, chunk_input in enumerate(chunks)])
                    # regular input
                    else:
                        input_chunks.append(IndexedTranslatorInputDoc(trans_input_idx,
                                                                      chunk_idx=0,
                                                                      translator_input=trans_input.with_eos()))
                else:
                    if trans_input.max_len_incl_context > self.max_input_length:
                        # oversized input
                        logger.debug(
                            "Input %s has length (%d) that exceeds max input length (%d). "
                            "Splitting into chunks of size %d.",
    ...
    def _get_inference_input(self,
                             trans_inputs: List[TranslatorInputDoc]) -> \
            Tuple[mx.nd.NDArray,
                  List[mx.nd.NDArray],
                  List[mx.nd.NDArray],
                  List[mx.nd.NDArray],
                  ...
                  mx.nd.NDArray]:
        """
        Assembles the numerical data for the batch. This comprises an NDArray for the source sentences, NDArrays
        for all additional context sentences,
        the bucket key (using padded lengths), and a list of raw constraint lists, one for each sentence in the
        batch - excluding context sentences, and finally an NDArray of maximum output
        lengths for each current source sentence in the batch.
        Each raw constraint list contains phrases in the form of lists of integers in the target language vocabulary.

        :param trans_inputs: List of TranslatorInputDocs.
        :return NDArray of source ids (shape=(batch_size, source_seq_len, num_factors)),
                list of NDArrays for each previous source sentence with its ids (shape=(batch_size, src_pre_length_i, 1)),
                list of NDArrays for each next source sentence with its ids (shape=(batch_size, src_nxt_length_j, 1)),
                list of NDArrays for each previous target sentence with its ids (shape=(batch_size, tar_pre_length_k, 1)),
                list of NDArrays for each next target sentence with its ids (shape=(batch_size, tar_nxt_length_m, 1)),
                bucket key,
                list of raw constraint lists,
                list of phrases to avoid,
                and an NDArray of maximum output lengths.
        """
        batch_size = len(trans_inputs)
        bucket_key_src_cur = data_io.get_bucket(max(len(inp.tokens) for inp in trans_inputs), self.buckets_source)
        source = mx.nd.zeros((batch_size, bucket_key_src_cur, self.num_source_factors), ctx=self.context)
        raw_constraints = [None] * batch_size  # type: List[Optional[constrained.RawConstraintList]]
        raw_avoid_list = [None] * batch_size  # type: List[Optional[constrained.RawConstraintList]]

        window_config = self.models[0].config.doc_context_config.window_config
        bucket_key_src_pre = []  # List[int]
        bucket_key_src_nxt = []  # List[int]
        bucket_key_tar_pre = []  # List[int]
        bucket_key_tar_nxt = []  # List[int]

        for i in range(window_config.src_pre):
            max_length = max(len(inp.src_pre_tokens[i]) for inp in trans_inputs)
            key = data_io.get_bucket(max_length, self.buckets_source)
            if key is None:
                key = max_length
            bucket_key_src_pre.append(key)

        for i in range(window_config.src_nxt):
            max_length = max(len(inp.src_nxt_tokens[i]) for inp in trans_inputs)
            key = data_io.get_bucket(max_length, self.buckets_source)
            if key is None:
                key = max_length
            bucket_key_src_nxt.append(key)

        for i in range(window_config.tar_pre):
            max_length = max(len(inp.tar_pre_tokens[i]) for inp in trans_inputs)
            key = data_io.get_bucket(max_length, self.buckets_target)
            if key is None:
                key = max_length
            bucket_key_tar_pre.append(key)

        for i in range(window_config.tar_nxt):
            max_length = max(len(inp.tar_nxt_tokens[i]) for inp in trans_inputs)
            key = data_io.get_bucket(max_length, self.buckets_target)
            if key is None:
                key = max_length
            bucket_key_tar_nxt.append(key)

        bucket_key = data_io.BucketDoc(src_bkt=bucket_key_src_cur,
                                       tar_bkt=-1,
                                       src_pre_bkts=bucket_key_src_pre,
                                       src_nxt_bkts=bucket_key_src_nxt,
                                       tar_pre_bkts=bucket_key_tar_pre,
                                       tar_nxt_bkts=bucket_key_tar_nxt)

        source_pre = [mx.nd.zeros((batch_size, bkt_key_src_pre, 1), ctx=self.context)
                      for bkt_key_src_pre in bucket_key_src_pre]
        source_nxt = [mx.nd.zeros((batch_size, bkt_key_src_nxt, 1), ctx=self.context)
                      for bkt_key_src_nxt in bucket_key_src_nxt]
        target_pre = [mx.nd.zeros((batch_size, bkt_key_tar_pre, 1), ctx=self.context)
                      for bkt_key_tar_pre in bucket_key_tar_pre]
        target_nxt = [mx.nd.zeros((batch_size, bkt_key_tar_nxt, 1), ctx=self.context)
                      for bkt_key_tar_nxt in bucket_key_tar_nxt]

        max_output_lengths = []  # type: List[int]
        for j, trans_input in enumerate(trans_inputs):
            num_tokens = len(trans_input)
            max_output_lengths.append(self.models[0].get_max_output_length(data_io.get_bucket(num_tokens,
                                                                                              self.buckets_source)))
            source[j, :num_tokens, 0] = data_io.tokens2ids(trans_input.tokens, self.source_vocabs[0])

            for i in range(window_config.src_pre):
                tokens = trans_input.src_pre_tokens[i]
                num_tokens = len(tokens)
                if num_tokens > 0:
                    source_pre[i][j, :num_tokens, 0] = data_io.tokens2ids(tokens, self.source_vocabs[0])

            for i in range(window_config.src_nxt):
                tokens = trans_input.src_nxt_tokens[i]
                num_tokens = len(tokens)
                if num_tokens > 0:
                    source_nxt[i][j, :num_tokens, 0] = data_io.tokens2ids(tokens, self.source_vocabs[0])

            for i in range(window_config.tar_pre):
                tokens = trans_input.tar_pre_tokens[i]
                num_tokens = len(tokens)
                if num_tokens > 0:
                    target_pre[i][j, :num_tokens, 0] = data_io.tokens2ids(tokens, self.vocab_target)

            for i in range(window_config.tar_nxt):
                tokens = trans_input.tar_nxt_tokens[i]
                num_tokens = len(tokens)
                if num_tokens > 0:
                    target_nxt[i][j, :num_tokens, 0] = data_io.tokens2ids(tokens, self.vocab_target)
        ...
        return (source,
                source_pre, source_nxt,
                target_pre, target_nxt,
                bucket_key, raw_constraints,
                raw_avoid_list, mx.nd.array(max_output_lengths, ctx=self.context, dtype='int32'))

    def _make_result(self,
                     trans_input: TranslatorInputDoc,
                     translation: Translation) -> TranslatorOutput:
        """
        Returns a translator result from generated target-side word ids, attention matrices and scores.
        Strips stop ids from translation string.
    ...
    def _translate_nd(self,
                      ...
                      max_output_lengths: mx.nd.NDArray) -> List[Translation]:
        """
        Translates source of source_length with additional context data, given a bucket_key.

        :param source: Source ids. Shape: (batch_size, bucket_key.src_len, num_factors).
        :param source_pre: List of previous source sentences and its integer ids.
               Shape: (batch_size, bucket_key.src_pre_bkts[i], 1)
        :param source_nxt: List of next source sentences and its integer ids.
               Shape: (batch_size, bucket_key.src_nxt_bkts[j], 1)
        :param target_pre: List of previous target sentences and its integer ids.
               Shape: (batch_size, bucket_key.tar_pre_bkts[k], 1)
        :param target_nxt: List of next target sentences and its integer ids.
               Shape: (batch_size, bucket_key.tar_nxt_bkts[m], 1)
        :param bucket_key: Bucket key with length data for all inputs.
        :param raw_constraints: A list of optional constraint lists.
    ...
    def _encode(self,
                sources: mx.nd.NDArray,
                source_pre: List[mx.nd.NDArray],
                source_nxt: List[mx.nd.NDArray],
                target_pre: List[mx.nd.NDArray],
                target_nxt: List[mx.nd.NDArray],
                bucket_key: data_io.BucketDoc) -> List[ModelState]:
        """
        Returns a ModelState for each model representing the state of the model after encoding the source and the context
        sentences.

        :param sources: Source ids. Shape: (batch_size, bucket_key.src_len, num_factors).
        :param source_pre: List of integer ids for each previous source sentence. Shape: (batch_size, src_pre_len_i, 1)
        :param source_nxt: List of integer ids for each next source sentence. Shape: (batch_size, src_nxt_len_j, 1)
        :param target_pre: List of integer ids for each previous target sentence. Shape: (batch_size, tar_pre_len_k, 1)
        :param target_nxt: List of integer ids for each next target sentence. Shape: (batch_size, tar_nxt_len_m, 1)
        :param bucket_key: Bucket key.
        :return: List of ModelStates.
        """
        return [model.run_encoder(sources,
                                  source_pre,
                                  source_nxt,
                                  target_pre,
                                  target_nxt,
                                  bucket_key) for model in self.models]
    ...
    def _decode_step(self,
        ...
        :return: (scores, attention scores, list of model states)
        """
        bucket_key.tar_len = step
        bucket_key = copy.copy(bucket_key)
    ...
    def _beam_search(self,
        ...
        :param target_nxt: List of next target sentences and its integer ids.
               Shape: (batch_size, bucket_key.tar_nxt_bkts[m], 1)
        :param bucket_key: Bucket key with lengths for all input data.
        :param raw_constraint_list: A list of optional lists containing phrases (as lists of target word IDs)
               that must appear in each output.
        :param raw_avoid_list: A list of optional lists containing phrases (as lists of target word IDs)
               that must NOT appear in each output.
        ...
        # Length of encoded sequence (may differ from initial input length)
        encoded_source_length = self.models[0].encoder.get_encoded_seq_len(bucket_key.src_len)
        utils.check_condition(all(encoded_source_length ==
                                  model.encoder.get_encoded_seq_len(bucket_key.src_len) for model in self.models),
                              "Models must agree on encoded sequence length")
        # Maximum output length
        max_output_length = self.models[0].get_max_output_length(bucket_key.src_len)
        ...
        # (0) encode source sentence, returns a list
        model_states = self._encode(source, source_pre, source_nxt, target_pre, target_nxt, bucket_key)
...
class TranslatorInsideDecoder:
    ...
    def __init__(self,
                 ...
                 beam_prune: float,
                 beam_search_stop: str,
                 models: List[InferenceModelInsideDecoder],
                 source_vocabs: List[vocab.Vocab],
                 target_vocab: vocab.Vocab,
                 nbest_size: int = 1,
                 restrict_lexicon: Optional[lexicon.TopKLexicon] = None,
        ...
        logger.info("Translator (Inside-decoder) (%d model(s) beam_size=%d beam_prune=%s beam_search_stop=%s "
                    "nbest_size=%s ensemble_mode=%s max_batch_size=%d buckets_source=%s avoiding=%d)",
                    len(self.models),
                    self.beam_size,
                    'off' if not self.beam_prune else "%.2f" % self.beam_prune,
